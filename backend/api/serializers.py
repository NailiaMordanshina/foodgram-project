...
from recipes.models import Tag, Recipe, RecipeIngredient,\
    Ingredient, Subscription, Favorites, ShoppingCart
...
class RecipeCreateSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if any(ingredient_data['amount']< 1 for ingredient_data
               in ingredients_data):
            raise serializers.ValidationError(
...
class UserSubscriptionSerializer(serializers.ModelSerializer):
    ...
    def get_recipes(self, obj):
        ...
        recipes = recipes[:int(recipes_limit)]\
            if recipes_limit else recipes
        serializers = RecipePartialSerializer(
            recipes, many=True,
            context={'request': self.context.get('request')}
        )
